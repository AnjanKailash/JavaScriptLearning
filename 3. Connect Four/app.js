document.addEventListener('DOMContentLoaded', () => {
	...
	for (var i=0, len = squares.length; i < len; i++) {
		(function (index) {
			...
			squares[i].onclick = function() {
				//if the square below your current square if taken, you can go ontop of it
	...
	function checkBoard() {
		//make const that shows all the winning arrays
		const winningArrays = [
		...
		for(let y = 0; y < winningArrays.length; y++) {
			...
			//now check those arays to see if they have the class of player-one
			if (square1.classList.contains('player-one') &&
				square2.classList.contains('player-one') &&
				square3.classList.contains('player-one') &&
				square4.classList.contains('player-one')) {

				//if they do, player 1 is the winner
				...
				//if they do, player 2 is the winner
